...
class AnalysisEngine:
    ...
    def run_tasks(self, tasks: list[dict]) -> dict[str, Any]:
        """
        根据任务列表执行指定的统计分析（供 agent_runner.py 使用）。
        :param tasks: task_planner.py 生成的可执行任务列表
        :return: 统计结果 JSON
        """
        self.results = {
            "meta": self._build_meta(),
        ...
        for task in tasks:
            ...
            dispatch_attr = self._METHOD_DISPATCH.get(method_name)
            if dispatch_attr is None:
                logger.warning("未知统计方法 '%s'，跳过任务 %d", method_name, task_id)
                continue

            try:
                executor = getattr(self, dispatch_attr)
                if method_name == "分布检验":
                    executor(variables[0], task_id)
                else:
                    executor(variables[0], variables[1], task_id)
    ...
    # ==================================================================
    # 任务执行方法（供 run_tasks 调度）
    # ==================================================================

    def _execute_t_test(self, num_col: str, cat_col: str, task_id: int) -> None:
        """执行分组独立两样本 Welch t 检验。"""
    ...
    def _execute_paired_t_test(self, col1: str, col2: str, task_id: int) -> None:
        """执行配对 t 检验。"""
    ...
    def _execute_anova(self, num_col: str, cat_col: str, task_id: int) -> None:
        """执行单因素方差分析（含 Tukey HSD 事后检验）。"""
    ...
    def _execute_chi_square(self, col1: str, col2: str, task_id: int) -> None:
        """执行卡方独立性检验。"""
        contingency_table = pd.crosstab(self.df[col1], self.df[col2])
        chi2_stat, p_val, dof, _expected = scipy_stats.chi2_contingency(contingency_table)
    ...
    def _execute_distribution_test(self, col: str, task_id: int) -> None:
